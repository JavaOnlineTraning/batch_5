...
public class PatientController {
	...
	@GetMapping(value = "/disease/{patientDisease}")
	public ResponseEntity<List<PatientEntity>> getAllPatientByDisease(@PathVariable("patientDisease") String patientDisease) {
		List<PatientEntity> entity = patientService.getAllPatientByDisease(patientDisease);
		return ResponseEntity.status(HttpStatus.ACCEPTED).body(entity);
	}
	
	
	@GetMapping(value = "/name/{patientName}")
	public ResponseEntity<List<PatientEntity>> getAllPatientByName(@PathVariable("patientName") String patientName) {
		List<PatientEntity> entity = patientService.getAllPatientByName(patientName);
		return ResponseEntity.status(HttpStatus.ACCEPTED).body(entity);
	}
}