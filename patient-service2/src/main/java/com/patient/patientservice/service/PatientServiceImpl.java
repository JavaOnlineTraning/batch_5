...
//		 PatientEntity patientEntity2 = findById.get();
	...
	@Override
	public List<PatientEntity> getAllPatientByDisease(String patientDisease) {
		List<PatientEntity> patientEntity = patientRepo.findAll();
		List<PatientEntity> list=patientEntity.stream().filter(s->s.getPatientDisease().equalsIgnoreCase(patientDisease)).collect(Collectors.toList());
		return list;
	}

	@Override
	public List<PatientEntity> getAllPatientByName(String patientName) {
		// TODO Auto-generated method stub
		List<PatientEntity> findAllByName = patientRepo.findAllByPatientName(patientName);
